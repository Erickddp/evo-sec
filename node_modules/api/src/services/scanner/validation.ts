...
export function isAllowedTarget(target: string): boolean {
    ...
    if (ipPart !== cleanTarget) {
        if (ipPart === 'localhost' || ipPart === '127.0.0.1') return true;
        if (ipPart.startsWith('192.168.')) return true;
        if (ipPart.startsWith('10.')) return true;
        if (/^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(ipPart)) return true;
    }
